(function (root, nodeFactory, browserFactory) {
...
function createPrivateStateStore(rootPath) {
  const directory = import_node_path.default.join(rootPath, ".sync-core");
  const statePath = import_node_path.default.join(directory, "state.json");
  if ((import_node_fs.default.statSync(rootPath).mode & 146) === 0) {
  ...
  if (import_node_fs.default.existsSync(directory)) {
    ...
    try {
      import_node_fs.default.mkdirSync(directory, { mode: 448 });
    } catch (error) {
      throw new Error(`Sync runtime cannot create private state directory: ${error.message}`);
    }
  }
  try {
    import_node_fs.default.chmodSync(directory, 448);
    const mode = import_node_fs.default.statSync(directory).mode & 511;
    if ((mode & 128) === 0) {
      throw new Error("directory is read-only");
    }
    import_node_fs.default.accessSync(directory, import_node_fs.default.constants.W_OK);
  } catch (error) {
  ...
  function loadState() {
    ...
    try {
      if (import_node_fs.default.lstatSync(statePath).isSymbolicLink()) {
        throw new Error("symbolic link at state.json");
      }
      fileDescriptor = import_node_fs.default.openSync(statePath, import_node_fs.default.constants.O_RDONLY | import_node_fs.default.constants.O_NOFOLLOW);
      if (!import_node_fs.default.fstatSync(fileDescriptor).isFile()) {
        throw new Error("state.json is not a regular file");
      }
      parsed = JSON.parse(import_node_fs.default.readFileSync(fileDescriptor, "utf8"));
    } catch (error) {
  ...
  function saveState(state) {
    ...
    const temporaryPath = import_node_path.default.join(
      directory,
    ...
    try {
      temporaryDescriptor = import_node_fs.default.openSync(
        temporaryPath,
      ...
      import_node_fs.default.writeFileSync(temporaryDescriptor, `${JSON.stringify(validated, null, 2)}
`, "utf8");
      import_node_fs.default.fchmodSync(temporaryDescriptor, 384);
      import_node_fs.default.fsyncSync(temporaryDescriptor);
      import_node_fs.default.closeSync(temporaryDescriptor);
      temporaryDescriptor = null;
      import_node_fs.default.renameSync(temporaryPath, statePath);
      directoryDescriptor = import_node_fs.default.openSync(directory, import_node_fs.default.constants.O_RDONLY | import_node_fs.default.constants.O_DIRECTORY);
      try {
        import_node_fs.default.fsyncSync(directoryDescriptor);
      } catch (error) {
      ...
      if (directoryDescriptor !== null) {
        import_node_fs.default.closeSync(directoryDescriptor);
      }
      try {
...
function normalizeRuntimeOptions(options) {
  ...
  const rootPath = import_node_path2.default.resolve(options.rootPath);
  let rootStat = null;
  try {
    rootStat = import_node_fs2.default.lstatSync(rootPath);
  } catch (error) {
    const code = error !== null && typeof error === "object" ? error.code : null;
    ...
    try {
      import_node_fs2.default.mkdirSync(rootPath, { recursive: true, mode: 448 });
      rootStat = import_node_fs2.default.lstatSync(rootPath);
    } catch (mkdirError) {
