...
function isSyncConfigMisconfigured(config: any = {}) {
    return config.enabled === true && !config.remoteUrl;
}

function isTransientPersistedStatus(status: any) {
    return status === 'syncing' || status === 'route_after_sync';
}

function normalizeState(config: any, storedState: any = {}) {
...
function createSyncRuntime(options: any = {}) {
...
function createSyncRuntimeFromResolvedOptions(normalized: any) {
    let {
        config,
        sourceRoot,
        ignorePatterns,
    ...
    async function runSyncPipeline(context: any = {}) {
        try {
            backend.ensureReady();
    ...
    async function requestSync(context: any = {}, transitionName: any = 'SYNC_REQUESTED') {
        if (inFlight) {
            return inFlight;
        }
    ...
    return {
        async notifyLocalMutation(context: any = {}) {
