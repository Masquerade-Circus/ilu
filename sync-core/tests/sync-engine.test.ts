...
const coreRuntimeOptionsModulePath = path.join(repoRoot, 'sync-core', 'runtime-options.ts');
const coreGitCliBackendModulePath = path.join(repoRoot, 'sync-core', 'backends', 'git-cli.ts');
const coreFileStateStoreModulePath = path.join(repoRoot, 'sync-core', 'state', 'file-store.ts');
const consumerIndexModulePath = path.join(repoRoot, 'sync', 'index.ts');
const consumerAdapterModulePath = path.join(repoRoot, 'sync', 'ilu-adapter.ts');
const consumerBackendModulePath = path.join(repoRoot, 'sync', 'git-cli-backend.ts');
const consumerStateStoreModulePath = path.join(repoRoot, 'sync', 'state-store.ts');

function loadEngine() {
  delete (require.cache as any)[require.resolve(engineModulePath)];
  return require(engineModulePath);
}

function loadAdvanced() {
  delete (require.cache as any)[require.resolve(advancedModulePath)];
  return require(advancedModulePath);
}

function loadEngineWithCoreStubs({gitCliBackendExports, fileStoreExports}: any = {}) {
  const restoredEntries = [];

  function stubModule(modulePath, exports) {
    const resolvedPath = require.resolve(modulePath);
    restoredEntries.push([resolvedPath, (require.cache as any)[resolvedPath]]);
  ...
  try {
    delete (require.cache as any)[require.resolve(coreDefaultsModulePath)];
  } catch (error) {
  ...
  return {
    ...
    restore() {
      ...
      restoredEntries.reverse().forEach(([modulePath, previousEntry]) => {
        if (previousEntry) {
          (require.cache as any)[modulePath] = previousEntry;
        } else {
...
function loadEngineWithRuntimeStubs({runtimeOptionsExports, defaultsExports}: any = {}) {
  const restoredEntries = [];
...
function loadSyncConsumer({engineExports, adapterExports, backendExports, stateStoreExports}) {
  ...
  delete (require.cache as any)[require.resolve(consumerIndexModulePath)];
...
function createFlatRuntimeScenario(overrides: any = {}) {
  const events = [];
...
function loadFlatRuntimeWithScenario(overrides: any = {}) {
...
test('sync engine preserves original error when backend classification omits error object', async () => {
  const harness = createHarness({
    dependencies: {
      backend: {
        ensureReady() {},
        ...
        hasChanges() { return true; },
        commit() {},
        fetch() { throw new Error('backend fetch exploded'); },
        integrate() {},
        push() {},
        getStatus() { return '## main'; },
        classifyGitError() {
  ...
  assert.equal(status.status, 'degraded_network');
  assert.equal(status.lastErrorKind, 'network');
  assert.equal(status.lastErrorMessage, 'backend fetch exploded');
});
