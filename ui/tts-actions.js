...
function callProgress(onProgress, message) {
  if (typeof onProgress === 'function') {
    onProgress(message);
  }
}

function isSupportedVoice(voice, voices) {
  return typeof voice === 'string' && voices.includes(voice);
}

function hasConfiguredCredentials(readStoredApiKey) {
  try {
...
function isSupportedInputFile(inputFile, fileSystem) {
...
function createTtsActions({
  ...
  const supportedVoices = Array.isArray(voices) ? voices.filter(voice => typeof voice === 'string' && voice.trim().length > 0) : [];
