...
function normalizePositions(value: unknown): number[] {
  ...
  return [...new Set(value.filter((item): item is number => typeof item === "number" && Number.isInteger(item) && item > 0))].sort((left, right) => left - right);
}
...
export function initialFocusForClockOverlay(state: ClockRuntimeState): string | null {
  if (state.overlay === "add-clock") return "clock-add-name";
  if (state.overlay === "remove-clock-confirm") return "clock-remove-confirm";
  ...
  function clockRows(): JSX.Element[] {
    ...
    return [
      <List
        id="clock-items"
        items={items}
        itemKey={(clock: ClockItem, index: number) => String(clockPosition(clock, index))}
        showActive={true}
        virtualized={true}
        height={6}
        ...
        wrap={true}
      >
        {(clock: ClockItem, ctx: { index: number }) => renderClockLabel(clock, ctx.index)}
      </List>
    ];
  }
      ...
      <AppOverlay trapFocus={true} content={[
          <FocusScope>
            ...
            <List
              ...
              items={choices}
              itemKey={(choice: TimezoneChoice) => choice.value}
              showActive={true}
              virtualized={true}
            ...
            >
              {(choice: TimezoneChoice) => choice.name}
            </List>
          </FocusScope>
