...
function normalizeWipLimitInput(value) {
...
function findBoardById(model, id) {
  if (typeof model.get === 'function') {
    return model.get(id);
  }

  if (typeof model.find === 'function') {
    return model.find().find(board => board && (board.$id === id || board.id === id || board.index === id));
  }

  return null;
}

function ensureCurrentBoardAfterRemove(model) {
...
function safeModelError(fallback) {
  return {ok: false, error: fallback};
}

function createBoardActions(options = {}) {
  const injectedModel = options.model;
  const modelFor = () => injectedModel || loadBoardModel();

  function withCurrentBoard(callback, missingMessage, failureMessage) {
    try {
  ...
  return {
    ...
    setWipLimit(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.columns.edit(columnIndex, {wipLimit: wipLimit.value}),
    ...
    setDefaultColumn(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.columns.setDefault(columnIndex),
    ...
    addCard(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.cards.add({title, description}),
    ...
    editCard(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.cards.edit({columnIndex, position, values: {title, description}}),
    ...
    moveCard(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.cards.move({fromColumn, fromPosition, toColumn}),
    ...
    prioritizeCard(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.cards.move({fromColumn: columnIndex, fromPosition: position, toColumn: columnIndex, toPosition}),
    ...
    removeCard(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.cards.remove({columnIndex, positions: [position]}),
    ...
    renameColumn(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.columns.edit(columnIndex, {title}),
    ...
    moveColumnLeft(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.columns.reorder({fromIndex: columnIndex, toIndex: columnIndex - 1}),
    ...
    removeColumn(values = {}) {
      ...
      const result = withCurrentBoard(
        model => model.columns.remove(columnIndex),
