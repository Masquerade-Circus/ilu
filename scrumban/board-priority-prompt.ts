...
function createState({columnTitle, cards, selectedPosition}: any) {
  return {
    columnTitle,
...
function swapCards(cards: any, leftIndex: any, rightIndex: any) {
...
function getCardPrefix(state: any, index: any) {
...
function render(state: any) {
  ...
  return [
    ...
    instructions,
    '',
    ...state.cards.map((card: any, index: any) => `${getCardPrefix(state, index)} ${index + 1}. ${card.title}`)
  ].join('\n');
}
...
async function promptBoardPriority({columnTitle, cards, selectedPosition, input = process.stdin, output = process.stdout}: any = {}) {
