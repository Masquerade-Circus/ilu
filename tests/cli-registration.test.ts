...
function runUiUntilFirstFrame(env) {
  return new Promise(resolve => {
    const child = spawn(process.execPath, [cliBinPath, 'ui'], {
      cwd: repoRoot,
      env: {...process.env, ...env},
    ...
    child.stdout.on('data', chunk => {
      ...
      if (stdout.includes('Todo')) {
        clearTimeout(timeout);
        child.kill('SIGTERM');
        finish({timedOut: false, code: 0});
      }
    });
...
test('bin/cli.js usa el tsconfig del paquete para ejecutar la TUI', async () => {
  ...
  fs.writeFileSync(
    hostileTsconfigPath,
    JSON.stringify({compilerOptions: {jsx: 'react'}}, null, 2)
  );

  const result = await runUiUntilFirstFrame({TSX_TSCONFIG_PATH: hostileTsconfigPath});

  assert.equal(result.timedOut, false);
  assert.match(result.stdout, /Todo/);
  assert.doesNotMatch(result.stderr, /React is not defined/);
});
