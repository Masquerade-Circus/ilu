...
async function main() {
  ...
  try {
    let localPaths = require(path.join(repoRoot, 'utils', 'local-paths.ts'));
    let result;
    ...
    if (flowName === 'note') {
      ...
      const NotesModel = require(path.join(repoRoot, 'notes', 'model.ts'));
    ...
    if (flowName === 'board') {
      ...
      const ScrumbanModel = require(path.join(repoRoot, 'scrumban', 'model.ts'));
