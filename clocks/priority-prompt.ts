...
function cloneClocks(clocks: any = []) {
  return clocks.map((clock: any) => ({...clock}));
}

function createState({clocks, selectedPosition = 1}: any) {
  return {
...
function swapClocks(clocks: any, leftIndex: any, rightIndex: any) {
...
function updatePendingMove(state: any) {
...
function reducePriorityPrompt(state: any, key: any) {
...
function getClockPrefix(state: any, index: any) {
...
function render(state: any) {
  ...
  return [
    ...
    instructions,
    '',
    ...state.clocks.map((clock: any, index: any) => `${getClockPrefix(state, index)} ${index + 1}. ${clock.name} (${clock.timezone})`)
  ].join('\n');
}
...
async function promptClockPriority({clocks, input = process.stdin, output = process.stdout}: any = {}) {
