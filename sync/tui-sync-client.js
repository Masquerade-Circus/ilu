...
function createTuiSyncClient(options = {}) {
  const fork = options.fork || defaultFork;
  const runnerPath = options.runnerPath || path.join(__dirname, 'tui-sync-runner.js');
  ...
  if (typeof child.on === 'function') {
    child.on('message', (message) => {
      ...
      if (message.type === 'sync:result' && typeof payload.id === 'string') {
        settle(payload.id, function settleResolve(value) { this.resolve(value.status || value); }, payload);
        return;
      }

      if (message.type === 'sync:error' && typeof payload.id === 'string') {
        settle(payload.id, function settleReject(value) { this.reject(new Error(value.message || 'Sync failed')); }, payload);
      }
    });
